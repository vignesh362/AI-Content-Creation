...
import textwrap
import asyncio
import edge_tts  # New: using edge-tts for natural TTS
...
# ---------- BOT SPEECH & BUBBLE ----------
def speak_line(text):
    # Use edge-tts for a more natural and masculine voice.
    def run_tts():
        async def tts_task():
            # "en-US-GuyNeural" is a natural-sounding, masculine voice.
    ...
    # Run TTS in a separate thread so it doesn't block the main loop.
...
def main():
    ...
    # Create a resizable transcript window
    ...
    # Add a click cooldown to debounce multiple clicks (in seconds)
    click_cooldown = 0.3
    last_click_time = 0
    ...
    # Setup ElfKing.
    # Change the entrance/exit directions so the character enters from off-screen
    # and later leaves in the same direction (i.e. if user_side==1, enter from left and exit to right)
    if user_side == 1:
        start_x = -50
    ...
    elf_introduced = False  # Flag for introduction complete.
    exit_phase = False      # Flag for exit phase.
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if button_rect.collidepoint(event.pos):
                    # Only increment if we have another dialogue line available.
                    if current_index < total_lines - 1:
                        ...
                        # Reset flags for dialogue transition
                        elf_introduced = True
                        exit_phase = False
        ...
        if ret:
            ...
            # Entrance phase: animate from off-screen until the character reaches center.
            ...
            # During the script, keep the character at center.
            elif elf_introduced and current_index < total_lines:
                elf_king.x = VIDEO_WIDTH // 4
                elf_king.update(AnimationState.TALKING)
            # Exit phase: after the last dialogue, walk off the screen.
            elif current_index >= total_lines:
                if not exit_phase:
                    exit_phase = True
                    # Set exit target based on leave_direction.
                    if leave_direction == 1:
                        exit_target = VIDEO_WIDTH + 50
                    else:
                        exit_target = -(elf_king.rect.width + 50)
                    # Set the character's direction to leave_direction.
                    elf_king.direction = leave_direction
                # Animate exit toward exit_target.
                if leave_direction == 1:
            ...
            # Draw speech bubble for bot dialogues.
            if current_index < total_lines:
                ...
                if speaker.lower() == "bot":
                    ...
                    # Adjust bubble_y to be closer to the character.
        ...
        divider_y = transcript_window_height - 250  # space for suggestions area.
        ...
        suggestions_area_top = divider_y + 10
        suggestions_area_bottom = button_y - 10
        with suggestion_lock:
            display_suggestion = suggestion_for_current_line if suggestion_for_current_line else "No suggestions available."
        suggestion_title = suggestion_font.render("Suggestions:", True, HEADER_COLOR)
        transcript_screen.blit(suggestion_title, (margin_left, suggestions_area_top))
        line_height = suggestion_font.get_height() + 2
        sug_y = suggestions_area_top + suggestion_title.get_height() + 5
        suggestion_lines = textwrap.wrap(display_suggestion, width=wrap_limit_suggestions)
        for sug_line in suggestion_lines:
            if sug_y + line_height > suggestions_area_bottom:
